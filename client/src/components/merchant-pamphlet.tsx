...
export function MerchantPamphlet({ 
  ...
  const handlePrint = async () => {
    try {
      ...
      // Add footer with branding - moved higher to avoid overlap with step 6
      yPosition += 10; // Add some extra space after warning box
      ...
      doc.text(`Generated by CPXTB Platform | ${new Date().toLocaleDateString()}`, pageWidth / 2, yPosition + 4, { align: "center" });
      doc.setTextColor(100, 100, 100);
      doc.text(`${businessName}`, pageWidth / 2, yPosition + 8, { align: "center" });
